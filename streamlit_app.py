    ...
    if "hf_email" not in st.session_state or "hf_pass" not in st.session_state:
        with st.expander("ℹ️ Login in Hugging Face", expanded=True):
            ...
            if st.button("Login 🚀") and hf_email and hf_pass and hf_token:
                with st.spinner("🚀 Logging in..."):
                    ...
                    try:
                        sign = Login(
                            st.session_state["hf_email"], st.session_state["hf_pass"]
                        )
                        cookies = sign.login()
                        ...
                        st.warning(
                            "⚠️ If you don't have an account, you can register [here](https://huggingface.co/join)."
                        )
                    ...
                    if "generated" not in st.session_state:
                        st.session_state["generated"] = [
                            "I'm **IA GUI chat**, How may I help you ? "
                        ]
                    ## past stores User's questions
                    if "past" not in st.session_state:
                        st.session_state["past"] = ["Hello!"]

                    st.session_state["LLM"] = HuggingChat(
                        email=st.session_state["hf_email"],
        ...
        with st.expander("ℹ️ Advanced Settings"):
            ...
            temperature = st.slider(
                "🌡 Temperature", min_value=0.1, max_value=1.0, value=0.5, step=0.01
            )
            top_p = st.slider(
                "💡 Top P", min_value=0.1, max_value=1.0, value=0.95, step=0.01
            )
            ...
            max_new_tokens = st.slider(
                "📝 Max New Tokens", min_value=1, max_value=1024, value=1024, step=1
            )
        ...
        if "plugin" not in st.session_state:
            ...
            if st.session_state["plugin"] == "🛑 No PLUGIN":
                st.session_state["plugin"] = st.selectbox(
                    "🔌 Plugins",
                    plugins,
                    index=plugins.index(st.session_state["plugin"]),
                )

        # WEB SEARCH PLUGIN
        if (
            st.session_state["plugin"] == "🌐 Web Search"
            and "web_search" not in st.session_state
        ):
            # web search settings
            with st.expander("🌐 Web Search Settings", expanded=True):
                if (
                    "web_search" not in st.session_state
                    or st.session_state["web_search"] == False
                ):
                    ...
                    if "region" not in st.session_state:
                        ...
                        st.session_state["region"] = st.selectbox(
                            "🗺 Region", reg, index=reg.index(st.session_state["region"])
                        )
                    if "safesearch" not in st.session_state:
                        st.session_state["safesearch"] = st.selectbox(
                            "🚨 Safe Search", sf, index=1
                        )
                    else:
                        st.session_state["safesearch"] = st.selectbox(
                            "🚨 Safe Search",
                            sf,
                            index=sf.index(st.session_state["safesearch"]),
                        )
                    if "timelimit" not in st.session_state:
                        st.session_state["timelimit"] = st.selectbox(
                        ...
                        st.session_state["timelimit"] = st.selectbox(
                            "📅 Time Limit",
                            tl,
                            index=tl.index(st.session_state["timelimit"]),
                        )
                    if "max_results" not in st.session_state:
                        st.session_state["max_results"] = st.slider(
                            "📊 Max Results", min_value=1, max_value=5, value=2, step=1
                        )
                    else:
                        st.session_state["max_results"] = st.slider(
                            "📊 Max Results",
            ...
            st.session_state["plugin"] == "🌐 Web Search"
            and st.session_state["web_search"] == "True"
        ):
            with st.expander("🌐 Web Search Settings", expanded=True):
        ...
        if (
            ...
            and "god_mode" not in st.session_state
        ):
            with st.expander("🧠 GOD MODE Settings", expanded=True):
                if (
                    "god_mode" not in st.session_state
                    or st.session_state["god_mode"] == False
                ):
                    topic = st.text_input(
                    ...
                    website_result = st.checkbox(
                        "🔗 Website Search", value=True, disabled=True
                    )
                    deep_of_search = st.slider(
                        "📊 Deep of Search", min_value=1, max_value=5, value=2, step=1
                    )
                    if st.button("🧠✅ Give knowledge to the model"):
                        ...
                        for id in yt_ids:
                            try:
                                ...
                                with st.spinner(f"👨‍💻 Scraping YT video : {id}"):
                                    transcript_list = (
                                        YouTubeTranscriptApi.list_transcripts(id)
                                    )
                                    transcript_en = None
                                    last_language = ""
                                    ...
                                    if transcript_en is None:
                                        transcript_en = transcript_list.find_transcript(
                                            [last_language]
                                        )
                        ...
                        with st.spinner("🧠 Building vectorstore with knowledge..."):
                            ...
                            db = Chroma.from_documents(
                                texts,
                                embeddings,
                                persist_directory="./chroma_db_" + random_str,
                            )
                        ...
                        with st.spinner("🔨 Saving vectorstore..."):
                            ...
                            shutil.make_archive(
                                "./chroma_db_" + random_str,
                                "zip",
                                "./chroma_db_" + random_str,
                            )
                            # save in session state and download
                            st.session_state["db"] = (
                                "./chroma_db_" + random_str + ".zip"
                            )
                        ...
                        with st.spinner("🔨 Creating QA chain..."):
                            ...
                            st.session_state["god_mode_info"] = (
                                "🧠 GOD MODE have builded a vectorstore about **"
                                + topic
        ...
        if (
            st.session_state["plugin"] == "🧠 GOD MODE"
            and "god_mode" in st.session_state
        ):
        ...
        # DATA PLUGIN
        if (
            st.session_state["plugin"] == "📋 Talk with your DATA"
        ...
        # DOCUMENTS PLUGIN
        if (
            st.session_state["plugin"] == "📝 Talk with your DOCUMENTS"
        ...
        # AUDIO PLUGIN
        if (
            st.session_state["plugin"] == "🎧 Talk with your AUDIO"
            and "audio" not in st.session_state
        ):
        ...
        # YT PLUGIN
        if (
            st.session_state["plugin"] == "🎥 Talk with YT video"
            and "yt" not in st.session_state
        ):
                ...
                if yt_url is not None and st.button(
                    ...
                    if yt_url != "":
                        ...
                        for i in range(video):
                            with st.spinner(
                                f"🎥 Extracting TEXT from YouTube video {str(i)} ..."
                            ):
                                # get en subtitles
        ...
        # WEBSITE PLUGIN
        if (
            st.session_state["plugin"] == "🔗 Talk with Website"
            and "web_sites" not in st.session_state
        ):
        ...
        if (
            st.session_state["plugin"] == "💾 Upload saved VectorStore"
            and "old_db" not in st.session_state
        ):
            with st.expander("💾 Upload saved VectorStore", expanded=True):
                db_file = st.file_uploader("Upload a saved VectorStore", type=["zip"])
                if db_file is not None and st.button(
                    "✅💾 Add saved VectorStore to context"
                ):
                    if db_file != "":
                        with st.spinner("💾 Extracting VectorStore..."):
                            ...
                            # Create retriever interface
                            db = Chroma(
                                "chroma_db_" + random_str, embedding_function=hf
                            )
...
def generate_response(prompt):
    ...
    with loading_container:
        ...
        if (
            ...
            if (
                prompt.find("python") != -1
                or prompt.find("Code") != -1
                or prompt.find("code") != -1
                or prompt.find("Python") != -1
            ):
                with st.spinner("🚀 Using tool for python code..."):
                    solution = "\n```python\n"
                    solution += st.session_state["df"].sketch.howto(
                        prompt, call_display=False
                    )
                    solution += "\n```\n\n"
                    final_prompt = prompt4Code(prompt, context, solution)
            else:
                with st.spinner("🚀 Using tool to get information..."):
                    solution = st.session_state["df"].sketch.ask(
                        prompt, call_display=False
                    )
            ...
            st.session_state["plugin"] == "🔗 Talk with Website"
            and "web_sites" in st.session_state
        ):
            # get only last message
            context = f"User: {st.session_state['past'][-1]}\nBot: {st.session_state['generated'][-1]}\n"
            with st.spinner("🚀 Using tool to get information..."):
            ...
            st.session_state["plugin"] == "💾 Upload saved VectorStore"
            and "old_db" in st.session_state
        ):
            # get only last message
            context = f"User: {st.session_state['past'][-1]}\nBot: {st.session_state['generated'][-1]}\n"
            with st.spinner("🚀 Using tool to get information..."):
                    ...
                    with st.spinner("🚀 Using internet to get information..."):
                        ...
                        with DDGS() as ddgs:
                            ddgs_gen = ddgs.text(
                                ...
                                timelimit=st.session_state["timelimit"],
                            )
                        ...
                        final_prompt = prompt4conversationInternet(
                            prompt, context, internet_result, internet_answer
                        )
                else:
...
with response_container:
    ...
    if "generated" in st.session_state:
        ...
        st.error(
            "👉 If you are not registered on Hugging Face, please register first and then login 🤗"
        )
